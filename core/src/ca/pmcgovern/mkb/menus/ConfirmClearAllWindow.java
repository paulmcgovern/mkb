...
public class ConfirmClearAllWindow extends Window {

    private Container<Actor> payload;
    
    public static final String CLEAR_ALL_CONFIRM = "CLEAR_ALL_CONFIRM";
    
    public ConfirmClearAllWindow(String title, AssetManager assetMgr, int taskCount, ChangeListener clearAllListener ) {
       ...
       // String taskDesc = taskSprite.getTaskDescription();
        ...
        buttonTable.add( new Label( "Delete all " + taskCount + " tasks?", skin, "tiny" )).colspan( 2 );
        buttonTable.row();  
       ...
       // PlayClickListener playClick = new PlayClickListener( effectMgr );
        ...
        MkbScreen.layoutButton( itb ); 
        itb.setName( CLEAR_ALL_CONFIRM );
        buttonTable.add( itb ).padLeft( 10 );
        itb.addListener( closer ); 
        itb.addListener( clearAllListener );
    ...
    class CloseListener extends ChangeListener {
        ...
        public void changed(ChangeListener.ChangeEvent event, Actor actor) {
            ConfirmClearAllWindow.this.addAction( Actions.sequence( Actions.fadeOut( FADE_DURATION ),Actions.removeActor()));
        }
    }     
